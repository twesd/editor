...
// AngelScript signature:
// float parseFloat(const string &in val)
float parseFloat(const string &val)
{	
	return irr::core::fast_atof(val.c_str());
}
...
void RegisterStdString_Native(asIScriptEngine *engine)
{
	...
	r = engine->RegisterGlobalFunction("int parseInt(const string &in, uint base = 10, uint &out byteCount = 0)", asFUNCTION(parseInt), asCALL_CDECL); assert(r >= 0);
	r = engine->RegisterGlobalFunction("float parseFloat(const string &in)", asFUNCTION(parseFloat), asCALL_CDECL); assert(r >= 0);
